...
import { getUniGPAMeta } from "@/lib/seo";
...
export async function generateMetadata({ params }: { params: Promise<{ country: string; university: string }> }): Promise<Metadata> {
  ...
  // Pakistan unis: rich programmatic metadata from lib/seo.ts
  if (uniPK) return getUniGPAMeta(uniPK, countryId);
  // Global unis: basic metadata
  const country = getCountryById(countryId);
  const uniName = (uni as any).name;
  const title = `${uniName} GPA Calculator 2026 | Free GPA Calculator — Survive Uni`;
  const desc = `Free ${uniName} GPA calculator. Track your semester GPA, predict your finals, and plan academic goals for 2026.`;
  return {
    title, description: desc,
    alternates: { canonical: `/${countryId}/${uniId}/gpa-calculator` },
    openGraph: { title, description: desc, url: `https://surviveuni.online/${countryId}/${uniId}/gpa-calculator`, type: 'website' },
  };
}
...
export default async function UniGPAPage({ params }: { params: Promise<{ country: string; university: string }> }) {
  ...
  return (
    <>
      ...
      {/* Rich SEO content: schemas + grading table + dept context + probation + tips + links */}
      {isPK && uniPK ? (
        <UniSEOContent
          uni={uniPK}
          countryId={countryId}
